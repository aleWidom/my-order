...
async function getItemsAccordingToSelectedCategory(categorySelected: string) {
  try {
    ...
    const selectedCategory = allCategories.data.find(
      (e: CategoryRestaurant) => e.name === categorySelected
    );
    ...
    return allItemsIdRestaurant1.data.filter(
      (e: Plate) => e.id_category === selectedCategory.id
    );
  } catch (err) {
    console.log(err);
  }
...
async function getItemsResults(valueInput: string) {
  try {
    const searchResults = await axios.get(
      `https://18eqrnlodc.execute-api.us-east-1.amazonaws.com/dev/items?search=${valueInput}`
    );
...
async function getItemsResultsCardHome(cardTitle: string) {
  try {
    const searchResultsCardHome = await axios.get(
      `https://18eqrnlodc.execute-api.us-east-1.amazonaws.com/dev/items?search=${cardTitle}`
    );
...
async function getItemsCardsRanking() {
  try {
    const cardsRankingPlates = await axios.get(
      `https://18eqrnlodc.execute-api.us-east-1.amazonaws.com/dev/items?ranking`
    );
...
async function getItemsCardsDayPlates() {
  try {
    const cardsDayPlates = await axios.get(
      `https://18eqrnlodc.execute-api.us-east-1.amazonaws.com/dev/items?dayPlates`
    );
