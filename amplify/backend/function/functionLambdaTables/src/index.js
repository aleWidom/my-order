...
VALUES (${JSON.stringify(event.pathParameters.proxy.slice(0,36))},${JSON.stringify(event.pathParameters.proxy.slice(37,73))})`, function (error, results, fields) {
                    resolve(results)
                }); 
            })
            result = await promiseQuery
            ...
            const promiseQuery = new Promise((resolve) => {
                connection.query(`Select * from PeopleInTable Where id_table = ${JSON.stringify(event.queryStringParameters.peopleInTableSearch)} Order by entry Desc`, function (error, results, fields) {
                    resolve(results)
                });
            })
            result = await promiseQuery
